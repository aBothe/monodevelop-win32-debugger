...
namespace Extemory.Injection
{
    public class InjectedModule
    {
        ...
        internal InjectedModule(IntPtr baseAddress, string moduelName, string file, Process process)
        {
        ...
        {
            var manager = ProcessMemoryManager.ForProcess(_parentProcess);
            //if (!manager.InjectedModules.ContainsKey(ModuleName))            throw new Exception("The injected module does not appear to be associated to its parent process. This could indicate corrupted process state");
        ...
        private IntPtr FindExport(string functionName)
        {
            ...
            try
            {
                hModule = Imports.LoadLibraryEx(ModuleFile, IntPtr.Zero, LoadLibraryExFlags.DontResolveDllReferences);
